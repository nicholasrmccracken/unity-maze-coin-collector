...
namespace ShareefSoftware
{
    public class Maze : IGridGraph<Direction>
    {
        ...
        public Maze(int numberOfRows, int numberOfColumns, System.Random random, GridTraversal<int>.AlgorithmType algorithmType)
        {
            endRow = numberOfRows - 1;
            endColumn = numberOfColumns - 1;
            endToDirection = Direction.North;

            CreateMaze(numberOfRows, numberOfColumns, random, algorithmType);
            AddEntrance();
            AddExit();
        }

        private void CreateMaze(int numberOfRows, int numberOfColumns, System.Random random, 
            GridTraversal<int>.AlgorithmType algorithmType)
        {
            var grid = new GridGraphRandomizedNeighborsDecorator<int>(
                new GridGraph<int>(
                    numberOfRows, numberOfColumns, GraphValueAccessorConstant<int>.DefaultConstant), random);
            var mazeGenerator = new GridTraversal<int>(grid, algorithmType);

            mazeGrid = new GridGraph<Direction>(
                numberOfRows, 
                numberOfColumns, 
                new GridNodeDataStore<Direction>(numberOfRows, numberOfColumns));
